...
begin
  ...
  begin
    if (X >= (HalfScreenWidth * CellSize - 142)) and
      (X <= (HalfScreenWidth * CellSize + 142)) then
    begin
      PPM := (X - 340) div 35;
      if (PPM < 0) then
        PPM := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(BRC, Durability);
      if (BRC > 199) or (Durability = 0) then
      begin
        PL := 0;
        BRC := 0;
        with Game do
...
begin
  case Key of
    ...
    ord('L'):
      IsDrawInfo := not IsDrawInfo;
    ord('S'):
      IsGameSound := not IsGameSound;
    ord('I'), ord('E'):
      begin
        BRC := 0;
        fMain.Timer1.Enabled := False;
...
begin
  ...
  begin
    if (Y >= 252) and (Y <= 283) and (X >= 552) and (X <= 583) and
      (RCD.ItemID > 0) and (RCD.Count > 0) // and (Button = mbRight)
...
begin
  ...
  Game.Graphic.BG.Canvas.Rectangle((HalfScreenWidth * 32) - Round(W),
    (HalfScreenHeight * 32) - Round(H), (HalfScreenWidth * 32) + Round(W),
