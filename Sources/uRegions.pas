...
var
  RMap: array [0..(ScreenWidth * 3) - 1, 0..(ScreenHeight * 3) - 1, 0..2] of TCell;
...
uses Classes, SysUtils, Dialogs, uGame, Utils;
...
begin
  for Y := 0 to ScreenHeight - 1 do
    for X := 0 to ScreenWidth - 1 do
      for Z := 0 to 2 do
        RMap[X + (AX * ScreenWidth)]
          [Y + (AY * ScreenHeight)][Z] := Map[X][Y][Z];
end;
...
var
  ...
  begin
    ...
    begin
       if (Ord(Map[X][Y][Z].Block) = bWood + BS) then Exit;
  ...
  begin
    if (Rand(1, 9) > 1) then AddCell(X, Y, 0, bLeaves);
  ...
  var
    FX, FY: Integer;
  begin
    if (X < 2) or (X > ScreenWidth - 3) or (Y < 2) or (Y > ScreenHeight - 3) or (BaseLine < 1) then Exit;
    for FX := 0 to 4 do for FY := 0 to 4 do
    begin
      case Rand(1, 9) of
        1   : Map[X + (FX - 2)][Y + (FY - 2)][1].Block := Chr(BlockID + BS);
        2, 3: Map[X + (FX - 2)][Y + (FY - 2)][0].Block := Chr(BlockID + BS);
      end;
    end;
  end;
...
begin
  PP := 0; PG := HalfScreenHeight; PR := 0; PL := 0;
  if (Density = 0) then Density := Rand(1, 100);
  if (Rand(1, 2) = 1) then Inc(Density, 10) else Dec(Density, 10);
...
//  Density := 100;
    ...
    begin
      ...
      case BaseLine of
        ...
        begin
          ...
          begin
            if (X + PG + 1 > ScreenHeight) and (PG <> ScreenHeight div 2) then
              if (PG > ScreenHeight div 2) then Dec(PG) else Inc(PG) else
                if (Rand(1, 3) = 1) then
                  Inc(PG) else Dec(PG);
            B := False;
          end;
          ...
          begin
            ...
            if F then Block := bGravel;
            if (Rand(1, 7) = 1) then F := not F;
            if K then DeepBlock := bGravel;
            if (Rand(1, 7) = 1) then K := not K;
            if Odd(X) then
          end;
          if (Y > PP) then
          begin
        ...
        begin
          ...
          DeepBlock := bStone;
          case Rand(0, 555) of
            0..9: AddSpot(X, Y, bCoal);
          end;
          if (BaseLine > 1) then
          case Rand(0, 999) of
            0..2: AddSpot(X, Y, bIronOre);
          end;
        end;
      end;
      ...
      if (RegionPoint.X > 0) then PR := PG else PL := PG;
    end;
  end;
  // Add cactus
  if (BaseLine = 0) then
  for X := 1 to ScreenWidth - 2 do
    for Y := 1 to ScreenHeight - 2 do
      if (Ord(Map[X][Y][1].Block) = bSand + BS)
        and (Ord(Map[X][Y - 1][1].Block) = bSky + BS)
        and (Rand(0, 24) = 0) then
      begin
        C := Rand(1, 3);
  ...
  for X := 1 to ScreenWidth - 2 do
    for Y := 1 to ScreenHeight - 2 do
      if (Ord(Map[X][Y][1].Block) = bGrass + BS)
        and (Ord(Map[X][Y - 1][1].Block) = bSky + BS)
        and (Rand(0, 49) = 0) then
      begin
  ...
  for X := 1 to ScreenWidth - 2 do
    for Y := 1 to ScreenHeight - 2 do
      if (Ord(Map[X][Y][1].Block) = bGrass + BS)
        ...
        begin
          case Rand(1, 2) of
            1: AddCell(X, Y - 1, 2, bRose);
            2: AddCell(X, Y - 1, 2, bDandelion);
          end;
          Break;
        end;
  // Add papirus
  ...
  for X := 1 to ScreenWidth - 2 do
    for Y := 1 to ScreenHeight - 2 do
      if (Rand(0, 49) = 0) and (Ord(Map[X][Y][1].Block) = bGrass + BS)
        and (Ord(Map[X][Y - 1][1].Block) = bSky + BS) then
        begin
          C := Rand(1, 3);
          for D := Y - 1 downto Y - C - 1 do
            if (Map[X][D][1].Block = Chr(bSky + BS)) then
              Map[X][D][2].Block := Chr(bPapirus + BS) else Break;
          Break;
        end;
  // Add tree
  ...
  for X := 5 to ScreenWidth - 6 do
    for Y := 5 to ScreenHeight - 6 do
      ...
      if (Ord(Map[X + 1, Y - 1][1].Block) = bSky + BS) and (Ord(Map[X - 1][Y - 1][1].Block) = bSky + BS) then
      begin
        W := 1;
        C := Rand(5, 9);
        if (Y - C - 1 <= 0) then Continue;
        for D := Y - 1 downto Y - C do AddCell(X, D, 1, bWood);
        AddLeaves(X, Y - C - 1);
        if (Rand(1, 2) = 1) then AddLeaves(X, Y - C - 2);
        ...
        begin
          W := Rand(1, W);
          if (Rand(1, 2) = 1) then W := -W;
          if (Ord(Map[X + W, Y - 1][1].Block) = bSky + BS)
            and (Ord(Map[X + W, Y][1].Block) = bGrass + BS) then
            case Rand(1, 2) of
              1: AddCell(X + W, Y - 1, 2, bMushroom1);
              2: AddCell(X + W, Y - 1, 2, bMushroom2);
            end;
        end;
        Break;
      end;
  ...
  begin
    ...
    begin
      ...
      for X := ScreenWidth * AX to (ScreenWidth * (AX + 1)) - 1 do
        ...
        begin
          C := RMap[X][Y][Z].Block;
          if (Ord(C) < BS) then C := Chr(bSky + BS);
          S := S + C;
        end;
      F.Append(S);
    end;
    F.SaveToFile(SavePath + Format('%d~%d',
      [RegionPoint.X + AX - 1, RegionPoint.Y + AY - 1]));
