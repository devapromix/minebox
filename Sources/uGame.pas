  ...
  begin
    ...
    begin
      ...
      if (GetCell(HalfScreenWidth - 1 + AX, HalfScreenHeight, 1).Move = False)
        and (GetCell(HalfScreenWidth - 1 + AX, HalfScreenHeight + 1, 1)
        .Move = False) then
      begin
        Result := True;
        Exit
      end
      else if (GetCell(HalfScreenWidth - 1 + AX, HalfScreenHeight + 1, 1)
        .Move = False) and (GetCell(HalfScreenWidth - 1 + AX, HalfScreenHeight,
        1).Move = True) then
      begin
      ...
      begin
        for C := HalfScreenHeight + 3 to ScreenHeight - 1 do
          if (GetCell(HalfScreenWidth - 1 + AX, C, 1).Move = False) then
            Break;
        if (GetCell(HalfScreenWidth - 1 + AX, HalfScreenHeight + 2, 1)
          .Move = True) and (GetCell(HalfScreenWidth - 1 + AX, C, 1)
          .Move = False) then
        begin
          Self.Pos.X := Self.Pos.X + AX;
...
begin
  Result.Block := RMap[Self.Pos.X + ScreenWidth + AX, Self.Pos.Y + ScreenHeight
    + AY, AZ].Block;
...
begin
  RMap[Self.Pos.X + ScreenWidth + AX, Self.Pos.Y + ScreenHeight + AY, AZ].Block
    := Chr(BlockID + BS);
  if (GetCell(AX, AY + 1, AZ).BlockID = bGrass) then
    SetCell(AX, AY + 1, AZ, bDirt);
end;

function TGame.GetMoveCell(BlockID: Integer): Boolean;
begin
  case BlockID of
    bSky, bWood, bLeaves, bCactus, bPumpkin, bPapirus, bMushroom1, bMushroom2,
      bLadder, bRose, bDandelion, bTorch, bOpenDoor1, bOpenDoor2, bNet,
      bWaterMelon:
      Result := True;
  else
    Result := False;
...
begin
  ...
  begin
    case Rand(1, 2) of
      1:
        Sound.Play('close_door_1.wav');
      2:
        Sound.Play('close_door_2.wav');
    end;
  ...
  begin
    case Rand(1, 2) of
      1:
        Sound.Play('open_door_1.wav');
      2:
        Sound.Play('open_door_2.wav');
    end;
  ...
  // Put block
  if (PP[PPM].Count > 0) and (GetCell(AX, AY, AZ).BlockID = bSky) then
  begin
    ...
    case PP[PPM].ItemID of
      iDoor:
        if (GetCell(AX, AY, AZ).BlockID = bSky) and
...
begin
  ...
  begin
    ...
    begin
      if not AddItem(StrToInt(R[0])) then
        Exit;
  ...
  case GetCell(AX, AY, AZ).BlockID of
    bDoor1:
      if not AddItem(iDoor) then
        Exit;
    bWaterMelon:
      if not AddItem(iWaterMelon, Rand(5, 7)) then
        Exit;
    bLeaves:
      begin
        case Rand(1, 2) of
...
begin
  case BlockID of
    ...
    bSand, bGravel:
      case Rand(1, 2) of
        1:
          Sound.Play('dig_crumbly_1.wav');
        2:
          Sound.Play('dig_crumbly_2.wav');
      end;
    bStone, bCStone:
